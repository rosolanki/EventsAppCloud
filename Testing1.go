...
func (t *Testing1) Invoke(stub shim.ChaincodeStubInterface) peer.Response {
	...
	default:
		...
		return Error(http.StatusBadRequest, "Invoke Error: Invalid Function Call - Function does not exist")
...
func (t *Testing1) createParticipant(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	if len(args) < 2 {
		return Error(http.StatusBadRequest, "Invoke Error: Incorrect number of arguments - Two Argument expected")
	...
	if err != nil {
		return Error(http.StatusBadRequest, "Invoke Error (Create Participant):  Invalid Data - Check Payload")
	...
	if value, geterr := stub.GetState(strings.ToLower(participantID)); !(geterr == nil && value == nil) {
		return Error(http.StatusConflict, "Invoke Error (Create Participant): Participant Already Exists! Please Specify Another ID")
	...
	return Success(http.StatusOK, "Participant Created", nil)
	//return shim.Success(nil)
}
...
func (t *Testing1) getParticipant(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	if geterr != nil || value == nil {
		return Error(http.StatusInternalServerError, geterr.Error())
...
func (t *Testing1) deleteParticipant(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	return Success(http.StatusOK, "Participant Deleted", nil)
}
